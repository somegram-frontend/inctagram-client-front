import { FormSignIn, SignInForm } from '@/pages/ui/auth/signIn/formSignIn/FormSignIn'
import { useRouter } from 'next/router'
import { useLoginMutation } from '@/api/auth-api'
import { EnumTokens, loginArgs } from '@/api/auth-api.types'

const SignIn = () => {
  const router = useRouter()
  const [signIn, { isError, data, isSuccess, isLoading }] = useLoginMutation()
  const onSubmitSignIn = ({ email, password }: SignInForm) => {
    const signInData: loginArgs = {
      email,
      password,
    }
    signIn(signInData)
